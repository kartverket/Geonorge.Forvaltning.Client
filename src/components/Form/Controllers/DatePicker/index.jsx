...
export default function DatePicker({ field }) {
   const initDate = !isNil(field.value) && field.value !== '' ? new Date(field.value) : null;
   ...
   function handleClear() {
      setValue(null);
      field.onChange({ name: field.name, value: null });
   }

   function save() {
      originalDateRef.current = _value;
      field.onChange({ target: { name: field.name, value: dayjs(_value).format() }});
      datePickerRef.current.setOpen(false);
   }

   function cancel() {
      setValue(originalDateRef.current);
      field.onChange({ name: field.name, value: field.value });
      datePickerRef.current.setOpen(false);
   }
