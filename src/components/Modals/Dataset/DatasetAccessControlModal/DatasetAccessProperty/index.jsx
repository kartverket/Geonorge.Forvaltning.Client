...
export default function DatasetAccessProperty({ index, metadata }) {
   ...
   const { fields, append, remove } = useFieldArray({
      control,
      name: `accessByProperties.${index}.values`,
   });
   const selectedProperties = useWatch({ control, name: "accessByProperties" });

   const bottomValueRef = useRef(null);
         ...
         .filter(
            (data) =>
               ...
               !selectedProperties.some(
                  (property) => property.propertyId === data.Id
               )
         )
   ...
   function handlePropertyChange(event, field) {
      ...
      const property =
         metadata.find((data) => data.Id === target.value) || null;

      setValue(`accessByProperties.${index}.values`, [
         getDefaultValues(property.Id),
      ]);
   ...
   function addPropertyValue() {
      ...
      requestAnimationFrame(() => {
         if (!bottomValueRef.current) return;
         bottomValueRef.current.scrollIntoView({ behavior: "smooth" });
      });
   }
                  ...
                  <div key={field.id} className={styles.valueRow}>
                     ...
                     <div className={styles.buttons}>
                        ...
                        {fields.length > 1 && (
                           <gn-button>
                              <button
                                 onClick={() => removePropertyValue(valueIndex)}
