...
import styles from "./DatasetImportCsvModal.module.scss";

export default function DatasetImportCsvModal({ dataset, onClose }) {
   ...
   async function readFile(file) {
      try {
         ...
         Object.values(geomColumns)[0].forEach(
            (column) => delete firstRow[column]
         );
   ...
   async function handleImport() {
      const { objects } = mapCsvToObjects(
         csvRef.current,
         geomColumnsRef.current,
         mappings,
         importSrId,
         user
      );
   ...
   return (
      <div className={styles.modal}>
         ...
         <heading-text>
            <h2 underline="true">Importer CSV</h2>
         </heading-text>
               ...
               <div>
                  ...
                  <div className={styles.epsgSelect}>
                     <gn-select block="">
                        <select
                           value={importSrId}
                           onChange={(event) =>
                              setImportSrId(event.target.value)
                           }
                        >
                           {projections.map((projection) => (
                              <option
                                 key={projection.epsg}
                                 value={projection.srId}
                              >
                                 {projection.epsg}
                              </option>
                           ))}
                  ...
                  <gn-table>
                     <table className={styles.previewTable}>
                        ...
                        <tbody>
                           <tr>
                              {Object.entries(mappings).map((entry) => (
                                 <td key={entry[0]}>
