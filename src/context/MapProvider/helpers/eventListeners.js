...
export function setFeatureIdsInExtent(map) {
    ...
    const layerIdRegex = /^\d+-features$/;

    const layers = map.getLayers().getArray()
        .filter(layer => layerIdRegex.test(layer.get("id")));
        ...
        source.forEachFeatureInExtent(extent, (feature) => {
            ...
            if (Array.isArray(clusterFeatures)) {
                for (const clusterFeature of clusterFeatures) {
                    features.push(getFeatureIdAndDistance(clusterFeature, centerPoint));
                }
            } else {
                features.push(getFeatureIdAndDistance(feature, centerPoint));
            }
        });
    }
...
function getFeatureIdAndDistance(feature, centerPoint) {
