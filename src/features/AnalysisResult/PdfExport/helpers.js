...
export async function createMapImages(featureCollection) {
   ...
   const destinations = featureCollection.features.filter(
   ...
   for (let i = 0; i < destinations.length; i++) {
      ...
      const route = routes.find(
         (route) =>
            route.properties.destinationId === destination.properties.id.value
      );
...
async function createMapImage(start, destination, route) {
   ...
   return new Promise((resolve, reject) => {
      ...
      map.once("rendercomplete", () => {
         ...
         resolve({
            destinationId: route.properties.destinationId,
            image: base64,
         });
      });
   });
...
function createFeaturesLayer(start, destination, route) {
   ...
   const startFeature = reader.readFeature(start, {
      dataProjection: "EPSG:4326",
      featureProjection: "EPSG:3857",
   });
   startFeature.setStyle(
      createPointFeatureStyle({ color1: "#3767c7", color2: "#3767c75e" })
   );

   const destFeature = reader.readFeature(destination, {
      dataProjection: "EPSG:4326",
      featureProjection: "EPSG:3857",
   });
   destFeature.setStyle(
      createPointFeatureStyle({ color1: "#249446", color2: "#2494465e" })
   );

   const routeFeature = reader.readFeature(route, {
      dataProjection: "EPSG:4326",
      featureProjection: "EPSG:3857",
   });
