...
export default function PdfExport({ featureCollection, images }) {
   const { start, destinations } = useMemo(() => {
      const _start = featureCollection.features.find(
         (feature) => feature.properties._type === "start"
      );
      const _destinations = featureCollection.features.filter(
      ...
      inPlaceSort(_destinations).by({
         asc: (destination) => {
            const route = routes.find(
               (route) =>
                  route.properties.destinationId ===
                  destination.properties.id.value
            );
   ...
   function renderDestinations() {
      return destinations.map((destination, index) => (
         <View
            key={destination.properties.id.value}
            wrap={false}
            style={styles.object}
         >
            <Text style={styles.subTitle}>
               Treff {index + 1} av {destinations.length}:
            </Text>
   ...
   function renderRouteData(feature) {
      ...
      const route = featureCollection.features.find(
         (feature) =>
            feature.properties._type === "route" &&
            feature.properties.destinationId === featureId
      );
         ...
         <View style={styles.routeData}>
            <View style={styles.row}>
               ...
               <Text style={styles.value}>
                  {convertDistance(route.properties.distance)}
               </Text>
            </View>
            <View style={styles.row}>
               <Text style={styles.label}>Est. kjøretid:</Text>
               <Text style={styles.value}>
                  {convertDuration(route.properties.duration)}
               </Text>
            </View>
         </View>
      );
   ...
   function renderMapImage(feature) {
      const result = images.find(
         (img) => img.value?.destinationId === feature.properties.id.value
      );
      ...
      <Document>
         <Page size="A4" style={styles.page}>
            ...
            <Text style={styles.title}>
               Analyseresultat ({destinations.length} treff)
            </Text>
