...
export default function useFilters(nodes, metadata, options = {}) {
   ...
   const setFilters = useDebounce(
      props => {
         if (props) {
            _setFilters({ ...filters, [props.name]: { value: props.value, exact: props.exact } });
         } else {
            _setFilters({});
         }
      },
   ...
   const data = useMemo(
      () => {
         ...
         function getNodeValue(node, columnName) {
            ...
            const dataType = metadata.find(data => data.ColumnName === columnName)?.DataType || 'text';
