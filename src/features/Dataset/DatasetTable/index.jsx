...
export default function DatasetTable() {
   ...
   const _editedDataObjects = useSelector(
   ...
   const editedDataObjects = useMemo(() => {
      ...
      return _editedDataObjects.filter(
         (object) =>
            object.connectionId !== connectionId &&
            object.datasetId === definition.Id
      );
   }, [definition, _editedDataObjects, connectionId]);
   ...
   const sortFns = useMemo(() => {
      ...
      metadata.forEach((data) => {
         fns[data.Name.toUpperCase()] = (array) =>
            inPlaceSort(array).by((item) => item[data.ColumnName]);
      });
   ...
   function canEditObject(objectId) {
      let allowed = false;
      let dataObjectNode = data.nodes.find(
         (dataObject) => dataObject.id === objectId
      );

      if (definition.Organization === user.organization) allowed = true;

      if (
         definition.Contributors != null &&
         definition.Contributors.includes(user.organization)
      )
         allowed = true;

      if (definition.ForvaltningsObjektPropertiesMetadata) {
         definition.ForvaltningsObjektPropertiesMetadata.forEach((prop) => {
            prop.AccessByProperties.forEach((access) => {
               if (access.Contributors.includes(user.organization)) {
   ...
   function renderFormControl(name, value, dataType, objectId) {
      ...
      if (dataType === "text" && allowedValuesForProp !== null) {
         const options = allowedValuesForProp.map((option) => ({
            value: option,
            label: option,
         }));
   ...
   function renderRemoteEditor(objectId) {
      ...
      const dataObject = editedDataObjects.find(
         (dataObject) => dataObject.objectId === objectId
      );
   ...
   async function deleteRows() {
      const editedObjectIds = editedDataObjects.map(
         (dataObject) => dataObject.objectId
      );

      const toDelete = select.state.ids.filter(
         (id) =>
            visibleTableRowsRef.current.includes(id) &&
            !editedObjectIds.includes(id)
      );
      ...
      const { result } = await showModal({
         ...
         body: `Er du sikker på at du vil slette ${
            count === 1 ? "objektet" : `${count} objekter`
         }?`,
         okText: "Slett",
      });
   ...
   return (
      <>
         <div className={styles.container}>
               ...
               <div>
                  {canEdit() && (
                     <gn-button>
                        ...
                        >
                           {!editMode ? "Rediger tabell" : "Avslutt redigering"}
                        </button>
                     </gn-button>
                  )}
                  ...
                  <div className={styles.checkbox}>
                     <gn-input>
                        <input
                           ...
                           onChange={(event) =>
                              setShowOnlyInExtent(event.target.checked)
                           }
                        />
                     </gn-input>
                     <gn-label>
                        <label htmlFor="features-in-extent">
                           Vis kun objekter i kartutsnitt
                        </label>
                     </gn-label>
                  </div>
               </div>
            ...
            <Filters definition={definition} onChange={setFilters} />
            ...
            <gn-table hoverable="">
               ...
               >
                  {(tableList) => {
                     visibleTableRowsRef.current = tableList.map(
                        (item) => item.id
                     );
