...
const cacheUpdater = {
   deleteDataset: (dispatch, args) => {
      dispatch(
         api.util.updateQueryData('getDatasetDefinitions', undefined, cachedData => {
            const index = cachedData.findIndex(dataset => dataset.Id === args.id);
   ...
   deleteDatasetObjects: (dispatch, args) => {
      dispatch(
         api.util.updateQueryData('getDataset', args.tableId.toString(), cachedData => {
            const objects = cachedData.objects.filter(object => !args.ids.includes(object.id));
