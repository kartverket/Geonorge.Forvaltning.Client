...
function getContributors(definition, data) {
   ...
   const contributorsByProperty =
      definition.ForvaltningsObjektPropertiesMetadata.flatMap((property) => {
         const inputValue = data[property.ColumnName];

         return property.AccessByProperties.filter(
            (access) => access.Value === inputValue
