...
export function deleteFeatures(deletedDataObjects, map) {
...
export function updateDataObject(object, updatedDataObject) {
   const index = object.objects.findIndex(object => object.id === updatedDataObject.id);
...
export function deleteDataObjects(object, deletedDataObjects) {
   const objects = object.objects.filter(obj => !deletedDataObjects.includes(obj.id));
...
export function removeEmptyValues(object) {
   ...
   const clean = entries
      .map(([key, v]) => {
         const value = typeof v === 'object' && v !== null ? removeEmptyValues(v) : v;
         return [key, value];
      });
